...
const UserScore: FC<UserScoreProps> = ({ score, movie_id }) => {
  ...
  const { isWatchlist, toggleWatchlistClick } = useToggleWatchlist(movie_id);
  ...
  return (
    <div className="flex items-center gap-4 mt-4">
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <Button variant="ghost" onClick={toggleFavoriteClick}>
            {isFavorite ? <Heart color="#ff0e00" fill="#ff0e00" /> : <Heart />}
          </Button>
        </TooltipTrigger>
        <TooltipContent>
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <Button variant="ghost" onClick={toggleWatchlistClick}>
            {isWatchlist ? <Eye /> : <EyeOff />}
          </Button>
        </TooltipTrigger>
